...
class Vae1:
    ...
    def train_epoch(self, my_optimizer, data_loader):
        ...
        # save file:
        saveFile = open(self.reporter_file, 'w')
        ...
        for (i, data) in enumerate(data_loader):
            ...
            if (i in self.report_batch):
                print("Model's state parameters; epoch:" + str(i) + ". ", file=saveFile)
                for p in self.network.state_dict():
                    print(p, "\t", self.network.state_dict()[p].size(), self.network.state_dict()[p].data, file=saveFile)
            # -------------------------------------------------------------------------------------------------#
            # </editor-fold>
